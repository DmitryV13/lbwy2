...
public:
    ...
    explicit abstract_data_t(int capacity) : size(0), m_capacity(capacity), hashArray(nullptr), listOfNodes(nullptr) {
        haresize(m_capacity);
    };

    abstract_data_t(const int arr[], int length) : size(0), m_capacity(length * 2), hashArray(nullptr),
                                                   listOfNodes(nullptr) {
    ...
    abstract_data_t(const abstract_data_t &initObject) : size(initObject.size), m_capacity(initObject.m_capacity),
    ...
    abstract_data_t(iterator begin, iterator end): abstract_data_t(){
    ...
    //for integers
    size_t hashEvaluate(const int key) const {
...
//        throw std::runtime_error("put return");
    };
    ...
    static int is_node_equal(SpecialNode *object, abstract_data_t target) {
        if (target.at(object->m_key) == -3333) return 0;
        int x = 1, y = 1;
        if (object->left) {
    ...
    //search by the key
    int at(const int &key) const {//FIXME: throww exception
        ...
        return tmp == nullptr ? -3333 : tmp->m_value;
    };//what does it return
    ...
    void add(SpecialNode *&root, int key, int value, size_t hash) {
        if (root == nullptr) {
    ...
    void order(SpecialNode *node, int newMem, SpecialNode **nha, SpecialNode **nlon) {
        if (!node)
            return;
        if (node->left) {
    ...
    //clearing tree nodes
    void clearList(SpecialNode *nodeList) {
    ...
    bool contains(int key) {
        if (at(key) == -3333) return false;
        return true;
    };

    size_t count(int key) {
        if (at(key) == -3333) return 0;
        return 1;
    };
    ...
    abstract_data_t &operator=(const abstract_data_t &initObj) {
        abstract_data_t tmp(initObj);
        swap(tmp);
        tmp.clear();
        return *this;
    };
    ...
    friend std::ostream &operator<<(std::ostream &stream, const abstract_data_t &a) {
    ...
    friend std::istream &operator>>(std::istream &stream, abstract_data_t &a) {
        int tmp, i = 0;
        while (stream >> tmp) {
            a.put(i, tmp);
            i++;
...
bool operator==(const abstract_data_t &a, const abstract_data_t &b) {
    return abstract_data_t::is_equal(a, b);
}
